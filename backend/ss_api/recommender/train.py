...
# Initialize Django
import django
django.setup()  # This initializes Django and loads the settings

# Now import your models and other Django-related code
from ss_api.recommender.collaborative_model import CollaborativeFilteringModel, create_embedding_layer
...
class CollaborativeFilteringModel(tfrs.Model):
    def __init__(self, user_model, community_model, candidate_ids):
        ...
        # Ensure candidate IDs are in the correct format for FactorizedTopK
        # candidate_ids should be a tf.data.Dataset of strings
        self.candidates = tf.data.Dataset.from_tensor_slices(candidate_ids)  # This should already be strings
        self.candidates = self.candidates.batch(1)  # Properly batching the candidates
