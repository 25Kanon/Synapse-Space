    ...
    useEffect(() => {
        const fetchMembers = async () => {
            try {
                ...
                const memberExists = response.data.some((member) => parseInt(member.user_id) === parseInt(user.id));
    ...
    return (
        ...
        >
            <div className="h-full px-3 pb-4 my-3 overflow-y-auto rounded-lg bg-base-200">
                ...
                {/* If user is not a member, show message */}
                {!isMember ? (
                            ...
                            <li key={member.user_id}>
                                <div className="flex items-center w-full p-2 mt-3 rounded-full shadow bg-base-100 group">
                                    ...
                                    <span className="overflow-hidden text-sm ms-3 text-ellipsis whitespace-nowrap">
                                        <Link to={`/profile/user/${member.user_id}`}>{member.username}</Link>
                                    </span>
                                    <div className="ml-auto dropdown dropdown-end">
                                        ...
                                        >
                                            {member.user_id === String(user.id) ? (
                                                <div>
                                                    <li>
                                                        <Link to={`/profile/user/${member.user_id}`}>Profile</Link>
                                                    </li>
                                                </div>
                                            ) : (
                                                <>
                                                    <li>
                                                        <Link to={`/chat/${member.user_id}`}>Message</Link>
                                                    </li>
                                                    <li>
                                                        <button
                                                            onClick={() =>
